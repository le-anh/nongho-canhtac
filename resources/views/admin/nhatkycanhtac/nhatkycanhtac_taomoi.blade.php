...
<div class="container rounded">
  <div class="title">
    <h5 class="pull-left"><strong>NHẬT KÝ CANH TÁC <small> (tạo mới) </small></strong></h5>
    <a href="{{route('admin.nhatkycanhtac.trangchu')}}" class="btn btn-info pull-right"> <i class="fa fa-arrow-circle-left"></i></a>
  </div>
  <div>
    <form action="{{route('admin.nhatkycanhtac.luu')}}" method="post">
        @csrf()
        <div class="form-group">
            <label for="socanhtac_id"> Sổ canh tác id </label>
            <input type="text" name="socanhtac_id" id="socanhtac_id" class="form-control @error('socanhtac_id') is-invalid @enderror" value="{{old('socanhtac_id')}}" placeholder="Sổ canh tác id" autofocus>
		...
		<div class="form-group">
            ...
            <input type="text" name="giaidoanquytrinh_id" id="giaidoanquytrinh_id" class="form-control @error('giaidoanquytrinh_id') is-invalid @enderror" value="{{old('giaidoanquytrinh_id')}}" placeholder="Giai đoạn quy trình id" autofocus>
            @error('giaidoanquytrinh_id') <div class="alert alert-danger"> {{$message}} </div>  @enderror
        </div>
		<div class="form-group">
            <label for="ngaythuchien"> Ngày thực hiện </label>
            <input type="date" name="ngaythuchien" id="ngaythuchien" class="form-control @error('ngaythuchien') is-invalid @enderror" value="{{old('ngaythuchien')}}" placeholder="Ngày thực hiện" autofocus>
            @error('ngaythuchien') <div class="alert alert-danger"> {{$message}} </div>  @enderror
        </div>
		<div class="form-group">
            <label for="nguoithuchien"> Người thực hiện </label>
            <input type="text" name="nguoithuchien" id="nguoithuchien" class="form-control @error('nguoithuchien') is-invalid @enderror" value="{{old('nguoithuchien')}}" placeholder="Người thực hiện" autofocus>
            @error('nguoithuchien') <div class="alert alert-danger"> {{$message}} </div>  @enderror
        </div>
		<div class="form-group">
            <label for="tongchiphi"> Tổng chi phí </label>
            <input type="text" name="tongchiphi" id="tongchiphi" class="form-control @error('tongchiphi') is-invalid @enderror" value="{{old('tongchiphi')}}" placeholder="Tổng chi phí" autofocus>
